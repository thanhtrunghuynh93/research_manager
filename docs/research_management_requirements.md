...
Version 0.3 — proposed specification — 11 September 2026 (revised; see section 15)
...
The initial deployment is a private research workspace with one professor and multiple students. There are exactly two user roles, `prof` and `student`; the professor also manages workspace settings and invitations. Projects have multiple students, and students can participate in multiple projects. A project can have zero, one, or several repositories. The system must support literature review, theory, data preparation, implementation, experimentation, analysis, and paper writing.
...
| Create, archive, and configure projects | Yes | View assigned projects |
| Assign students and reporting obligations | Yes | View own assignments |
...
| Export records | Authorized workspace records | Own released records |

**AUTH-01 — Authentication.** Require authenticated accounts, invitation-based enrollment, session expiration, and account deactivation. Only the professor can change roles or project membership. Provide account recovery for every user through a verified out-of-band channel. Because the workspace has a single professor, the deployment must document an audited break-glass procedure for recovering or transferring the professor account; that procedure must not be reachable from a student account.

**AUTH-02 — Authorization.** Enforce workspace, project, ownership, and document-visibility permissions in every API, search, download, export, and AI retrieval operation. A project membership does not grant access to other students' private reports, individual assessments, or professor notes.
...
**PROJ-01 — Project records.** Store title, description, research questions, intended contributions, research stage, status, start and target dates, paper or venue targets when applicable, and shared resources. Statuses are proposed, active, paused, completed, and archived.

**PROJ-02 — Membership.** Record each student's responsibility, join and leave dates, first and last required reporting periods, planned allocation if used, and exemptions. Retain membership history instead of deleting it when a student leaves.
...
**UI-06 — Exports.** Export filtered reports, assessments, and supervision summaries with dates, source references, and approval status. Provide machine-readable export for portability; a printable report is desirable. Export authorization must match interactive access.

**UI-07 — Notifications.** Generate in-app notifications for report submission and resubmission, revision requests, released assessments and feedback, approaching and missed deadlines, overdue milestones, and repository sync or authorization failures. Deliver the missed-deadline reminder by email (REP-08); other categories go to email only when configured. Notification text must obey the recipient's visibility permissions and must never include private supervision notes or another student's report content. Record delivery state and let each user mute non-critical categories.
...
The API must cover users and memberships, projects and milestones, report drafts/submissions/revisions, repository connections and sync status, evidence inspection, assessments and review actions, professor queries, and authorized exports. Mutations must record actor and time; retryable submissions and external events must support idempotency.

## 11 Nonfunctional requirements

| Area | Proposed requirement and acceptance basis |
| --- | --- |
| Initial capacity | One workspace with up to 50 students, 30 active projects, and three years of reports; benchmark against at least 100,000 indexed evidence items |
...
| Data control | Professor-configured retention, export, and deletion; propagate authorized deletion to file storage, searchable indexes, answer caches, and derived records; document backup expiration |
...
| Portability | Export records and files with identifiers, versions, and relationships; support model-provider replacement without migrating the authoritative research history |
...
| --- | --- |
| MVP | Two roles; invitations and permissions; projects and memberships; basic milestones and frozen weekly plans; structured weekly reports with attachments and history; one repository provider; identity mapping; permission-filtered evidence search; draft assessments and professor approval; cited professor Q&A; dashboards; in-app reminders and automatic missed-deadline email; backup and export |
...
| AC-02 | A student on one project requests another student's private report or its citation URL | Access is denied in API, UI, search, cached responses, downloads, and exports |
