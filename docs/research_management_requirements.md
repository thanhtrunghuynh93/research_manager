...
Version 0.9 — proposed specification — 24 September 2026 (revised; see section 15)
...
A deployment holds one or more private research workspaces. **The two roles belong to them differently, and the asymmetry is the shape of the whole system:** a professor may belong to several workspaces at once, and a student belongs to exactly one. A professor reads and writes in the one they are working in, and switches between them; "working in one at a time" is a statement about what they see and where a write lands, not a limit on how many they may belong to. A student has one workspace, so for them the distinction does not arise.

Every record belongs to exactly one workspace, projects included — and a membership must agree with both ends, so a student is only ever on a project in their own workspace. That is not a rule a caller applies but a composite foreign key from `(workspace_id, project_id)` onto `projects(workspace_id, id)` and from `(workspace_id, student_id)` onto `users(workspace_id, id)`: the database refuses a membership that would join a student to a project across a workspace boundary, whichever workspace the professor happened to be working in when they asked.

Each workspace has one or more professors and multiple students. There are exactly two user roles, `prof` and `student`; professors also manage workspace settings and invitations. Professors are co-equal: each has the same authority over every student in the workspace, and none has authority over another (ADR 0011). Projects have multiple students, and students can participate in multiple projects. A project can have zero, one, or several repositories. The system must support literature review, theory, data preparation, implementation, experimentation, analysis, and paper writing.
...
| Assign students and reporting obligations | Yes, including ending any membership | Join a project opened to joining |
...
| Belong to several workspaces at once | Yes; reads and writes follow the one being worked in, and switching changes it | No — exactly one |

**AUTH-01 — Authentication.** Require authenticated accounts, invitation-based enrollment, session expiration, and account deactivation. Only a professor can enrol a user or remove a student from the workspace. A professor may place any student on any project and end any membership, their own students' included; a student may start a project and join one their professor has opened to joining, and may do neither on another account's behalf (PROJ-07). **Ending a membership is the professor's alone.** A student may not end their own: whether a piece of research is finished is the judgement the weekly meeting exists to make, and a student who has stopped owing a report is a student the professor is no longer told about ([ADR 0019](adr/0019-ending-a-membership-is-the-professors.md)). A user's role is fixed when they accept their invitation: any professor may invite a colleague as a professor, but no professor may demote, deactivate, or remove another through the application. Provide account recovery for every user through a verified out-of-band channel. The deployment must document an audited break-glass procedure — run from the host and not reachable from any account, student or professor — for recovering, transferring, demoting, or deactivating a professor account, and that procedure must refuse to leave a workspace with no active professor.
...
**AUTH-03 — Access changes.** Removing a membership or deactivating a user must invalidate subsequent access, including cached answers and download authorization. Preserve historical records for authorized supervision. Repository authorization and application visibility must be checked separately: access to a derived record must not bypass its source restrictions. A cached answer must be invalidated when access changes in any workspace its records came from, not only the one the request was made in (AUTH-05).

**AUTH-04 — Workspaces.** A workspace is the tenant boundary: every record belongs to exactly one — a project, a membership, a report, an assessment, a period — and no read crosses out of the set the caller belongs to. A project belongs to the workspace it was created in and never moves. A student's projects are therefore the projects of their one workspace, and the schema enforces it: `project_memberships` carries composite foreign keys onto `projects(workspace_id, id)` and `users(workspace_id, id)`, so a row joining a student to a project in a different workspace cannot be written at all. A professor may create a workspace, which they then own; rename it; set its timezone; and archive it once nobody belongs to it. A workspace is archived, never deleted, because every record in the schema points at one and deleting it would take the research history with it. An invitation names the workspace it enrols into, so every account has one from the moment it is created rather than from acceptance ([ADR 0012](adr/0012-workspace-ownership.md)).

**AUTH-05 — Belonging and scope.** A professor belongs to one or more workspaces and works in one of them at a time; a student belongs to exactly one. Belonging and working in are different things and the wording matters: a professor is not limited to one workspace, and "working in" names only where the next write lands. Switching is joining a workspace already belonged to. Reads and writes both follow the workspace the account is working in: a professor switching workspace changes what every screen shows, not only where the next write lands. Joining a workspace adds a membership and moves where writes land — joining one already belonged to therefore moves only the latter, which is how switching is expressed. Leaving gives up a membership and must be refused when it would leave active accounts in a workspace with no active professor (AUTH-01), or when it is the account's only membership ([ADR 0015](adr/0015-plural-workspace-membership.md), [ADR 0020](adr/0020-reads-follow-the-workspace-you-are-in.md), superseding [ADR 0016](adr/0016-reads-span-membership.md)).
...
**AUTH-07 — Project authorship.** The account that created a project may change its record — title, description, research questions, intended contributions, stage, and dates — for as long as the project exists, including after their membership on it has ended. It may not change the project's standing: status, whether the project is open to joining, and the restriction on sending its text to a model provider are the professor's alone. A professor may change any project in their workspace. A project with no recorded creator is the professor's alone to change.

## 3 Projects and research planning

**PROJ-01 — Project records.** A project belongs to exactly one workspace, fixed when it is created, and everything on it — tasks, decisions, memberships — belongs to that same workspace (AUTH-04). Store title, description, research questions, intended contributions, research stage, status, start and target dates, paper or venue targets when applicable, an optional link to the project's repository, and shared resources. **Shared resources are files attached to the project** rather than to a week: everyone on the project may read them and add them, whoever attached one may remove it, and they are stored, checksummed and kept like any other attachment but are not extracted, indexed or citable ([ADR 0018](adr/0018-project-documents-are-shared-with-the-project.md)). That link is a pointer for the people on the project and nothing more: it is not a connected repository (REPO-01), no events are read from it, and no contribution is attributed from it. Statuses are proposed, active, paused, completed, and archived. A project a professor creates is proposed and becomes active by an explicit change, because activation is where a second party's assent is recorded. A project a student creates is active on creation — there is no second party, and nothing is owed on a proposed project, so leaving it proposed would give them a project that produces nothing and no way to see why.

**PROJ-02 — Membership.** A membership joins a student to a project in the student's own workspace; the pair cannot straddle two, and the schema refuses it rather than trusting the caller (AUTH-04). Record each student's responsibility, join and leave dates, first and last required reporting periods, planned allocation if used, and exemptions. Retain membership history instead of deleting it when a membership ends. Record how each membership began — assigned by a professor, created with the project, or joined by the student — because they differ in which weeks they oblige. A membership that was assigned or created with the project owes the week it lands in: the professor knows what they are asking for, and a student starting a project is announcing work already under way. A student joining an existing project owes from the following week, so that joining on a Saturday is not a report due that Sunday for a week spent off the project. An obligation is derived as soon as the membership exists rather than waiting for the next scheduled run.

**PROJ-03 — Tasks. Amended in 0.8: milestones withdrawn.** Weekly tasks have planned outcomes, effort weights, completion criteria, and blockers. Completion may be partial and must include a reason and evidence. *Withdrawn:* milestones — an owner or contributors, a target date, status, success criteria, evidence, and the tasks that linked to them. They were built and never used: no screen created one, so none existed, and everything computed from them therefore reported on nothing. A task is what a plan baseline freezes (PROJ-04), which is why tasks stay.
...
**PROJ-06 — Project progress. Withdrawn in 0.8.** *Was: show milestone status, accepted deliverables, blockers, and timeline changes; compute percentage completion from professor-defined milestone weights and accepted completion fractions rather than from average student scores; retain and display the baseline version when milestone scope or weights change.* It went with the milestones it was computed from (PROJ-03). The rule it existed to state — that a project's progress is never an average of student scores — is not withdrawn and is restated in ASSESS-02, which keeps the four signals apart.

**PROJ-07 — Self-service membership.** A student may start a project, and may join any project in their workspace that a professor has marked open to joining. Joining is direct: there is no approval step, because the professor's decision was taken when they opened the project. A student may not end a membership — their own or anyone else's — and may not place any other account on a project. Ending one is the professor's, either directly or by taking the project out of `active`, which is the usual way a finished project ends: it stops every membership on it owing a week rather than needing a decision per student (REP-06, [ADR 0019](adr/0019-ending-a-membership-is-the-professors.md)). The asymmetry is deliberate — joining adds work to a student's week and ending one removes an obligation, and only the second is a supervision decision. Every membership records which of the two ways it began, and both are written to the audit log with the acting account as the actor. Opening a project to joining is a professor's decision per project, and the default is closed, because a membership grants the project's shared records and the identities of everyone who has worked on it (AUTH-02).
...
| Field | Requirement |
| --- | --- |
| Research stage | Select the stage the week's work belongs to |
...
| Evidence | Attach files; identify what each item supports |
...
| Next steps | Tasks and expected deliverables for next week, with outcomes and acceptance criteria |
...
Hours worked are not asked for. The field existed as an optional self-reported figure and was removed: it was never treated as verified productivity, nothing read it, and a number on a research report invites exactly the reading the rest of this specification refuses. Weeks that recorded one keep it, and it is shown as filed. Support English and Vietnamese report text and preserve technical terminology, equations, tables, and links.

**REP-04 — Editor and evidence.** Provide autosave, Markdown or rich-text editing, draft recovery, and file attachments. MVP inputs are text/Markdown, PDF, DOCX, figures, notebooks and data files. Evidence is a file, not a link: a fetched page can change or disappear after the week it was cited for, removal would delete our copy of something that is not the student's to delete, and a link and a file behaved differently in half the panel that listed them together. Links attached before this stay on the record and are shown as filed. Preserve original files and extracted text, mark extraction failures, read the text out of a file when the report it belongs to is submitted rather than as each file arrives — so attaching returns as soon as the bytes are safe, and a file attached and then removed before submitting is never read at all — and propose a configurable upload limit of 25 MB per file with a configurable total per project entry. The student who attached a file may remove it, submitted week or not, and removal is real: the stored object, the extracted text, the search index entries and the cached answers that could quote it all go (§11). An assessment written against a removed file keeps its rationale and loses the citation. OCR and specialist experiment integrations can follow later.
...
**REP-06 — Missing and exceptional weeks.** Record leave, holidays, paused projects, partial enrollment, and extensions. A project that is no longer `active` — paused, completed or archived — owes no report for any open week, **including the week in progress**: the same two tests that derive an obligation, a membership that lasts the week and a project that is active, decide whether one already derived is still owed. The obligation row is kept, because it is how the week was derived and the supervision history reads it, and it is filtered on the way out ([ADR 0019](adr/0019-ending-a-membership-is-the-professors.md)). An absent report without an approved exemption is recorded as a missing-report condition on the obligation; it does not create an invented research assessment or automatically assign a zero. Late reports retain their actual timestamps and may trigger a new assessment version.
...
4. Support needs, with evidence-linked explanations. *Amended in 0.8:* milestone status went with PROJ-03.
...
| --- | ---: | --- |
| Progress toward agreed outcomes | 30% | Meaningful advancement against the frozen plan, accounting explicitly for approved changes |
...
**ASSESS-10 — Trends and support flags.** Show recent weekly trajectories and repeated blockers. Suggested configurable flags include a missing report after the grace period, a blocker repeated for two reporting periods, and a report claim with unresolved evidence. Label these conditions directly; a discrepancy is a request for clarification. Preserve context such as leave, unavailable compute, changed scope, and supervisor dependencies. Do not infer misconduct, effort, or personal characteristics from sparse telemetry.
...
| How has Student B progressed over the last six weeks? | Dated plans, reports, assessments, and rubric comparability |
...
**UI-01 — Professor overview.** Show the current reporting week, missing/late submissions, a review queue, students needing discussion, and data-sync issues. Provide filters by student, project, research stage, and week. Keep evidence confidence visible beside assessments.
...
**UI-03 — Project workspace. Amended in 0.7, and again in 0.8.** Show research goals, members, the project's related documents (PROJ-01), repository connections, and — to the professor — the research stage. Individual records obey their own visibility controls. *Amended:* **milestones** and the completion figure computed from them are withdrawn outright (PROJ-03, PROJ-06). **Dated decisions** are recorded and not shown, because only `POST /{id}/decisions` writes one and no screen calls it; the read still answers and the panel is a screen's worth of work away from returning, the day the authoring side exists. **The stage** is the professor's view: it is the plan for the project rather than the student's account of it, and a student's other screens carry neither it nor anything computed from it.

**UI-04 — Student research profile.** For the professor, show the student's project history, weekly reports, distinct contributions, feedback, and unresolved support needs. Students see only the permitted subset of their own profile.
...
**UI-08 — Workspace and roll screens.** Show the professor the workspaces they belong to or own, with the actions AUTH-04 and AUTH-05 allow on each: work here, join, leave, create, archive, rename. Show the roll of the workspace being worked in — everyone who belongs to it — with invite, move, suspend, restore, and remove; moving names a destination among the workspaces the professor administers (AUTH-05).
...
**UI-07 — Notifications. Amended in 0.4: delivery only.** Generate notification records for report submission and resubmission, revision requests, released assessments and feedback, approaching and missed deadlines, and repository sync or authorization failures. Deliver the missed-deadline reminder by email (REP-08); other categories go to email only when configured. Notification text must obey the recipient's visibility permissions and must never include private supervision notes or another student's report content. Record delivery state. *Withdrawn from this requirement in 0.4: the in-app notification surface, and muting non-critical categories.* Email is therefore the only channel that reaches a recipient, which section 12 should be read against.
...
| Task | Project, assignee, planned outcome, acceptance criteria, effort weight, status, completion fraction and reason, blocker |
...
| Artifact and ArtifactVersion | File, checksum/version, extracted text, visibility, storage reference; attached to a week's entry or, with no period, to the project itself (PROJ-01). A URL is a historical value on rows filed before REP-04 withdrew links, and nothing fetches one |
...
The API must cover users and memberships, projects and tasks, report drafts/submissions/revisions, repository connections and sync status, evidence inspection, assessments and review actions, and professor queries. Exports were withdrawn in 0.4 (UI-06). Mutations must record actor and time; retryable submissions and external events must support idempotency.
...
| --- | --- |
| MVP | Two roles; invitations and permissions; projects and memberships; frozen weekly plans; structured weekly reports with attachments and history; one repository provider; identity mapping; permission-filtered evidence search; draft assessments and professor approval; cited professor Q&A; dashboards; workspaces and plural professor membership; the automatic missed-deadline email; backup |
...
| 0.7 | 21 September 2026 | Ending a membership is the professor's. **AUTH-01** and **PROJ-07** gave a student the right to end their own in 0.6 and take it back here: the other three rights that came with PROJ-07 — start a project, join an open one, edit what they started — are unchanged, and this is the fourth. Whether research is finished is a supervision judgement, and a student who stops owing a report is a student the professor is no longer told about. A professor ends one directly, or ends the project. **REP-06** now says what the second of those does: a project taken out of `active` owes no report for any open week, including the week in progress, which the implementation did not do — obligations already derived stayed on the student's week and in the missed-deadline email after the professor had called the project done. The obligation row is kept and filtered on the way out, as one whose membership ended mid-week already was. **PROJ-02** now says a membership ends rather than that a student leaves, and **AUTH-07** that a creator's right outlives the membership rather than the leaving ([ADR 0019](adr/0019-ending-a-membership-is-the-professors.md)). Also brought the rest of the specification up to what is built: **UI-03** now describes the project workspace as it is, and says which three things it no longer shows and why; **PROJ-06** names the professor as the reader of project progress; **PROJ-01** defines *shared resources*, which had been listed since 0.1 and never specified, as files attached to the project ([ADR 0018](adr/0018-project-documents-are-shared-with-the-project.md)); and section 9 stops calling an artifact a file *or URL*, since REP-04 withdrew links and nothing fetches one |
| 0.8 | 21 September 2026 | **Milestones are withdrawn** (**PROJ-03** amended, **PROJ-06** withdrawn), with everything computed from them. They were specified in 0.1, built, and never used: no screen ever created one, so every deployment held none, and the project-progress figure derived from `accepted_completion` — a column only an uncalled endpoint set — reported 0% for every project as though that were a finding. Migration 0026 drops `milestones`, `milestone_revisions`, `tasks.milestone_id` and `project_report_entries.milestone_ids`; five endpoints go with them. **Tasks stay**, because a plan baseline freezes tasks (PROJ-04) and they only ever pointed at a milestone optionally. The references went too: the REP-03 template no longer asks a student to link milestones, **ASSESS-02**'s fourth signal is support needs alone, the rubric's progress dimension measures advancement against the frozen plan rather than "milestone criteria", **ASSESS-10** drops the overdue-milestone flag, and **UI-01**, **UI-03**, **UI-04**, **UI-07**, **QA-01**, section 9 and section 12 no longer name them. What PROJ-06 existed to say — a project's progress is never an average of student scores — is restated in ASSESS-02 rather than lost |
| 0.9 | 24 September 2026 | **A professor's reads follow the workspace they are working in** (**AUTH-05**, **UI-08**, §1 and the §2 capability table amended). Since 0.4 a professor read across every workspace they belong to and wrote into one; the header switcher then presented the workspace it names as the frame every screen is read in, and it was not — switching left both labs' projects and cohorts on screen and moved only where the next project would land. Reads now follow the switch ([ADR 0020](adr/0020-reads-follow-the-workspace-you-are-in.md), superseding [ADR 0016](adr/0016-reads-span-membership.md)). Belonging stays plural, writes are unchanged, and moving a student still names its destination. **AUTH-03**'s cached-answer rule is restated without assuming a read spans |
