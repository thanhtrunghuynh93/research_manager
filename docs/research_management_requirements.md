...
Version 0.7 — proposed specification — 21 September 2026 (revised; see section 15)
...
**PROJ-06 — Project progress.** Show milestone status, accepted deliverables, blockers, and timeline changes to the professor (UI-03: a student's project view carries neither the stage nor the milestones). If showing percentage completion, compute it from professor-defined milestone weights and accepted completion fractions. Do not treat average student scores as a project's completion percentage. When milestone scope or weights change, retain and display the baseline version.
...
**UI-03 — Project workspace. Amended in 0.7.** Show research goals, members, the project's related documents (PROJ-01), repository connections, and — to the professor — the research stage and the milestones. Individual records obey their own visibility controls. *Amended:* three things this originally named are not on the screen. **Milestone completion** and **dated decisions** were removed because nothing in the product writes them: `accepted_completion` is set only by an endpoint no screen calls, so completion read 0% for every project as though that were a finding, and decisions are professor-only to record with no screen to record them. Both reads still answer and both panels are a screen's worth of work away from returning, the day the authoring side exists. **Stage and milestones** are the professor's view: they are the plan for the project rather than the student's account of it, and a student's other screens carry neither.
