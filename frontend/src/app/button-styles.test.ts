/**
 * Every `btn-*` class a component uses must exist in the stylesheet.
 *
...
test("no component uses a button class the stylesheet does not define", () => {
  const defined = new Set(
    [...readFileSync("src/index.css", "utf8").matchAll(/\.(btn-[a-z-]+)\s*\{/g)].flatMap(
      (match) => match[1] ?? [],
    ),
  );
  ...
  for (const file of filesUnder("src", ".tsx")) {
    for (const match of readFileSync(file, "utf8").matchAll(/\b(btn-[a-z-]+)\b/g)) {
      const name = match[1];
